         ...
         <div class="p-3">
            @foreach ($memos as $memo)
               <a href="{{ route('edit', ['id' => $memo->id]) }}"
                  class="block mb-2 hover:font-semibold truncate">{{ $memo->content }}</a>
            @endforeach
         </div>
      </section>
   </x-slot>
   ...
   <section class="text-gray-600 body-font overflow-hidden rounded-lg border border-gray-300">
      ...
      <div class="p-3">
         <form action="{{ route('update', ['id' => $edit_memo->id]) }}" method="POST">
            ...
            @csrf
            <div class="mb-3">
               <textarea class="w-full rounded" name="content" rows="5" placeholder="ここにメモを入力">{{ $edit_memo->content }}</textarea>
            </div>
            ...
            <button type="submit"
               class="text-white bg-indigo-500 border-0 py-1 px-4 focus:outline-none hover:bg-indigo-600 rounded text-base">更新</button>
         </form>
         ...
         <div class="mt-3 flex justify-end">
            ...
            <div class="mr-2">
               <form action="{{ route('destroy', ['id' => $edit_memo->id]) }}" method="POST">
                  ...
                  @csrf
                  <button type="submit"
                     class="text-white bg-red-500 border-0 py-1 px-4 focus:outline-none hover:bg-red-600 rounded text-base">メモを削除</button>
               </form>
            </div>
         </div>
      </div>
